...
/**
 * This is the implementation of a table view following Google's material design
 * guidelines in JavaFX.
 * <p>
 * Extends {@code Control} and provides a new skin since it is built from
 * scratch.
 *
...
public class MFXTableView<T> extends Control {
    ...
    private final FunctionProperty<T, MFXTableRow<T>> tableRowFactory = new FunctionProperty<>(
            item -> new MFXTableRow<>(this, item));

    private final TransformableListWrapper<T> transformableList = new TransformableListWrapper<>(
     ...
     * <p>
     * Uses {@link MFXTableRow#updateRow()} on the currently built rows,
     * {@link SimpleVirtualFlow#getCells()}.
     */
    public void update() {
    ...
    protected void layoutChildren() {
        ...
        if (!isVirtualFlowInitialized() && rowsFlow.getCellHeight() > 0)
            virtualFlowInitialized.set(true);
    }
    ...
    /**
     * @return the list that is effectively used by the {@link SimpleVirtualFlow}
