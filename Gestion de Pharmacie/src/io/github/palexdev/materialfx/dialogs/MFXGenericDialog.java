 ...
 * <p>
 * - the header: which is an {@link HBox} containing an icon(optional) and a
 * {@link Label}
 * to show the {@link #headerTextProperty()}. It also contains three buttons to
 * minimize, close and
 * set always on top. All three of them can be hidden. By default they don't do
 * anything, they have been
 * added since this dialog in indented to be used in conjunction with
 * {@link MFXStageDialog}, see
 * {@link MFXGenericDialogBuilder#toStageDialogBuilder()} for an example on how
 * those buttons are used
 * <p>
 * - the content: which is just a {@link Label} to show the
 * {@link #contentTextProperty()}, the
 * content can be easily changed in many ways: you can set the property
 * {@link #contentProperty()},
 * you can override {@link #buildContent()} or you can override
 * {@link #buildScrollableContent(boolean)}.
 * By default the dialog uses the content built by {@link #buildContent()}. The
 * other method, {@link #buildScrollableContent(boolean)},
 * by default is just wrapping the {@link #buildContent()} result in a
 * {@link MFXScrollPane}
 * <p>
 * - the footer/actions: which is a {@link HBox} or {@link VBox} (depending on
 * the {@link #actionsOrientationProperty()}),
 * which contains a list of arbitrary nodes specified by the user through:
 * {@link #addActions(Node...)} or {@link #addActions(Map.Entry[])},
 * you can also clear them with {@link #clearActions()}. Note that despite being
 * generic {@link Node}s, it is intended to be used
 * with buttons and the like
 * <p>
 * </p>
 * This dialog also offers a property to indicate whether the dialog is set to
 * be always on top or not,
 * {@link #alwaysOnTopProperty()}, also has a new related {@link PseudoClass}:
 * ":always-on-top".
 */
public class MFXGenericDialog extends AbstractMFXDialog {
	 ...
	 * <p>
	 * - updates the ":always-on-top" PseudoClass when
	 * {@link #alwaysOnTopProperty()} changes
	 * <p>
	 * - updates the center node when {@link #contentProperty()} changes
	 * <p>
	 * - re-initializes the header when one of these properties change:
	 * {@link #showAlwaysOnTopProperty()},
	 * {@link #showMinimizeProperty()}, {@link #showCloseProperty()}
	 * <p>
	 * - adds/removes event handlers for the always on top, minimize and close
	 * icons, specified by:
	 * {@link #onAlwaysOnTopProperty()}, {@link #onMinimizeProperty()},
	 * {@link #onCloseProperty()}
	 * <p>
	 * - re-builds the actions pane when {@link #actionsOrientationProperty()}
	 * changes
	 */
	private void initialize() {
		pseudoClassStateChanged(ALWAYS_ON_TOP_PSEUDO_CLASS, isAlwaysOnTop());
		alwaysOnTopProperty()
				.addListener(invalidated -> pseudoClassStateChanged(ALWAYS_ON_TOP_PSEUDO_CLASS, isAlwaysOnTop()));
		...
		onAlwaysOnTopProperty().addListener((observable, oldValue, newValue) -> {
			if (oldValue != null)
				alwaysOnTopIcon.removeEventHandler(MouseEvent.MOUSE_CLICKED, oldValue);
			if (newValue != null)
				alwaysOnTopIcon.addEventHandler(MouseEvent.MOUSE_CLICKED, newValue);
		});
		onMinimizeProperty().addListener((observable, oldValue, newValue) -> {
			if (oldValue != null)
				minimizeIcon.removeEventHandler(MouseEvent.MOUSE_CLICKED, oldValue);
			if (newValue != null)
				minimizeIcon.addEventHandler(MouseEvent.MOUSE_CLICKED, newValue);
		});

		onCloseProperty().addListener((observable, oldValue, newValue) -> {
			if (oldValue != null)
				closeIcon.removeEventHandler(MouseEvent.MOUSE_CLICKED, oldValue);
			if (newValue != null)
				closeIcon.addEventHandler(MouseEvent.MOUSE_CLICKED, newValue);
		});
	...
	private void initHeader() {
		...
		if (isShowAlwaysOnTop())
			header.getChildren().add(alwaysOnTopIcon);
		if (isShowMinimize())
	 ...
	 *
	 * @param smoothScrolling to specify whether to use smooth scrolling on the
	 *                        {@link MFXScrollPane}
	 */
	...
	/**
	 * Builds the actions pane according to the
	 * {@link #actionsOrientationProperty()}.
	 * <p>
	 * </p>
	...
	/**
	 * Each entry has a {@link Node} that will trigger the given action on
	 * {@link MouseEvent#MOUSE_CLICKED}.
	 * <p>
	 * For each entry, adds the given {@link EventHandler} to the given
	 * {@link Node}, than
