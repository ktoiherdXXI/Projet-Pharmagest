 ...
 * <p>
 * </p>
 * To make the scrim effect work, the show/close methods of {@link Stage} have
 * been
 * overridden, however the {@link #show()} method is final, to properly show the
 * dialog
 ...
 * with that is that creating Stages is a quite expensive task, cannot be done
 * on separate
 * threads dor whatever reason, and therefore can make the application
 * unresponsive,
 * even if for a short time.
 * <p>
 * So, the advice is to build this dialogs ahead of time, keep them in memory if
 * you can,
 * so that you can use them whenever you need and just change their content if
 * needed,
 */
public class MFXStageDialog extends Stage {
	...
	protected void scrimOwner() {
		...
		TimelineBuilder.build()
				.add(KeyFrames.of(200, scrimEffect.getScrimNode().opacityProperty(), getScrimStrength(),
	...
	protected void unScrimOwner() {
		if (ownerNode != null)
			scrimEffect.removeEffect(ownerNode);
		if (getOwner() != null)
			scrimEffect.removeEffect(getOwner());
		TimelineBuilder.build()
	...
	/**
	 * This is responsible for centering the dialog on the {@link #getOwnerNode()}
	 * (if enabled).
	 */
	protected void centerInOwner() {
		if (!isCenterInOwnerNode() || ownerNode == null)
			return;
	 ...
	 * Two notes:
	 * <p>
	 * - the file is loaded using the given class, so make sure the FXML file
	 * is in the right directory
	 * <p>
	 * - the method expects to load a {@link AbstractMFXDialog} as root
	 */
	...
	/**
	 * Same as {@link #load(Class, String)}, but in case of fail it will return
	 * null.
	 */
	...
	/**
	 * Specifies whether to scrim the {@link #getOwnerNode()} when showing the
	 * dialog.
	 */
	...
	/**
	 * Specifies the strength(opacity, so values from 0.0 to 1.0) of the scrim
	 * effect, by default it is 0.5
	 */
	 ...
	 *         You can have two owners, one is the stage owner(Window) and the other
	 *         is the dialog owner(Pane).
	 *         Sometimes it's better to apply the scrim to the window (for example
	 *         the owner node would not allow to apply the
	 *         scrim effect, for example AnchorPanes, VBoxes, HBoxes...), but you
	 *         still want to center the dialog in the owner node.
	 *         Setting this to {@link ScrimPriority#WINDOW} will tell the dialog to
	 *         apply the effect to {@link #getOwner()},
	 *         setting this to {@link ScrimPriority#NODE} will tell the dialog to
	 *         apply the effect to {@link #getOwnerNode()}.
	 */
